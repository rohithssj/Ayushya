...
trips_lex_count = sum(1 for r in lex_all if "trips" in str(r.chunk.get("domain","")).lower())
print("Results: %d  TRIPS in top-15: %d" % (len(lex_all), trips_lex_count))
...
trips_emb_count = sum(1 for r in emb_all if "trips" in str(r.chunk.get("domain","")).lower())
print("Results: %d  TRIPS in top-15: %d" % (len(emb_all), trips_emb_count))
for i, r in enumerate(emb_all[:15], 1):
    c = r.chunk
    is_t = "trips" in str(c.get("domain","")).lower()
    print("  sem#%d sim=%.4f  doc=%r  sec=%r  jur=%r  dom=%r%s" % (
        i, r.similarity_score, (c.get("title") or c.get("document_id") or "")[:40],
        str(c.get("section",""))[:25], c.get("jurisdiction",""), c.get("domain",""),
        " <<TRIPS>>" if is_t else ""))
...
print("\n[RRF jur=India (current /api/chat behavior -- why TRIPS is lost)]")
rrf_india = hybrid.search(query=trips_q, top_k=5, jurisdiction="India", domain=None)
print("Results: %d" % len(rrf_india))
...
print("\n[RRF jur=International dom=trips (correct filter for TRIPS query)]")
rrf_intl = hybrid.search(query=trips_q, top_k=5, jurisdiction="International", domain="trips")
for i, r in enumerate(rrf_intl, 1):
    c = r.chunk
    snip = str(c.get("text","")).lower()[:100]
    print("  rrf#%d hybrid=%.5f  doc=%r  sec=%r" % (
        i, r.hybrid_score, (c.get("title") or c.get("document_id") or "")[:40],
        str(c.get("section",""))[:25]))
    print("    text: %r" % snip)
...
def tm_classify(text):
    t = text.lower()
    flags = []
    if any(s in t for s in ["registrable","absolute ground","relative ground","not registrable","distinctive","class 5","class 30","eligib","application for registration"]): flags.append("REG/ELIG")
    if any(s in t for s in ["renewal","notice before removal"]): flags.append("REMOVAL/RENEWAL")
    if any(s in t for s in ["rectif"]): flags.append("RECTIFY")
...
for i, r in enumerate(lex_tm[:15], 1):
    c = r.chunk
    doc = (c.get("title") or c.get("document_id") or "")[:40]
    src = "ACT" if "act" in doc.lower() and "rules" not in doc.lower() else "RULES"
    print("  lex#%d score=%.4f  [%s]  sec=%r  [%s]" % (
        i, r.score, src, str(c.get("section",""))[:25], tm_classify(c.get("text",""))))
...
for i, r in enumerate(emb_tm[:15], 1):
    ...
    doc = (c.get("title") or c.get("document_id") or "")[:40]
    src = "ACT" if "act" in doc.lower() and "rules" not in doc.lower() else "RULES"
    print("  sem#%d sim=%.4f  [%s]  sec=%r  [%s]" % (
        i, r.similarity_score, src, str(c.get("section",""))[:25], tm_classify(c.get("text",""))))
...
for i, r in enumerate(rrf_tm, 1):
    ...
    doc = (c.get("title") or c.get("document_id") or "")[:40]
    src = "ACT" if "act" in doc.lower() and "rules" not in doc.lower() else "RULES"
    print("  rrf#%d hybrid=%.5f lex=%d sem=%d  [%s]  sec=%r  [%s]" % (
        i, r.hybrid_score, r.lexical_rank, r.semantic_rank,
        src, str(c.get("section",""))[:25], tm_classify(c.get("text",""))))
...
for c in tm_act:
    ...
    if any(s in text for s in ["registrable","absolute ground","relative ground","not registrable","distinctive","applicat"]):
        reg_chunks.append((sec, (c.get("title") or c.get("document_id") or "")[:35]))
print("Registration-related chunks in Act: %d" % len(reg_chunks))
for sec, title in reg_chunks[:10]:
...
if not api_key:
    ...
    print("API key: SET  Model:", os.environ.get("LLM_MODEL","nvidia/nemotron-3-super-120b-a12b"))
    ...
    combined = EvidenceEvaluator.evaluate(results=deduped,
        query="product analysis Ashwagandha Wellness Tablet",
        jurisdiction=request.jurisdiction, domain=None)
    print("\nCombined strength: %s  abstain: %s" % (combined.get("strength"), combined.get("abstention_recommended")))
    ...
    print("All selected: %d" % len(all_selected))
    ...
    print("LLM evidence items: %d  valid_cit_ids: %d" % (len(llm_evidence), len(valid_cit_ids)))
    print("  IDs:", sorted(valid_cit_ids))
    ...
    total_prompt = len(system_prompt) + len(user_prompt)
    print("Prompt chars: %d (sys=%d usr=%d)" % (total_prompt, len(system_prompt), len(user_prompt)))
    ...
    try:
        ...
        print("Markdown fence: %s" % has_fence)
        ...
        print("  (If this says 'model output must contain either output text or tool calls'")
        print("   -> OpenRouter empty-completion error, fallback should activate)")
    except LLMRateLimitError as e:
        print("LLMRateLimitError: %s" % e)
    except LLMTimeoutError as e:
        print("LLMTimeoutError: %s" % e)
    except LLMAuthError as e:
        print("LLMAuthError: %s" % e)
    except LLMProviderError as e:
        print("LLMProviderError[%s]: %s" % (type(e).__name__, e))

    if raw_answer is not None:
        parsed = parse_structured_analysis_response(raw_answer, valid_cit_ids)
        pf = parsed.get("_parse_fallback", False)
        fr = parsed.get("_fallback_reason", "")
        print("\nPARSER:")
        print("  _parse_fallback:", pf)
        if fr: print("  _fallback_reason:", fr)
        ...
        print("  tk_biodiversity:", "dict(keys=%s)" % list(parsed.get("tk_biodiversity",{}).keys()) if isinstance(parsed.get("tk_biodiversity"),dict) else repr(parsed.get("tk_biodiversity")))
        ...
        print("    Supplied to LLM: %d" % len(valid_cit_ids))
        print("    Found in response: %d" % len(all_cit_in_resp))
        print("    Matched valid: %d" % len(valid_matched))
        print("    Not in supplied set (hallucinated/misspelled): %d" % len(invalid_halluci))
        ...
        if pf:
            ...
            s = raw_answer.strip()
            if s.startswith("```"):
                print("  ROOT CAUSE: LLM wrapped response in markdown code fences")
            elif s.startswith("{"):
                try:
                    test = json.loads(s)
                    print("  json.loads OK - issue in _strip_markdown or _extract_json_object. Keys:", list(test.keys())[:10])
                except json.JSONDecodeError as je:
                    print("  JSON error at pos %d: %s" % (je.pos, je.msg))
                    print("  Context: %r" % s[max(0,je.pos-40):je.pos+60])
            else:
                print("  ROOT CAUSE: Response is not JSON. First 200:", repr(s[:200]))
        else:
            print("\n  Parse SUCCESS.")
            cls = parsed.get("classification") or {}
            if isinstance(cls, dict):
                print("  classification.evidence_strength:", cls.get("evidence_strength","?"))
                pa = cls.get("preliminary_assessment","")
                print("  classification.preliminary_assessment:", repr(str(pa)[:120]))
            ip = parsed.get("ip_assessment",[])
            for item in ip[:2]:
                print("  ip_item: relevance=%s strength=%s cit_count=%d" % (
                    item.get("relevance"), item.get("evidence_strength"),
                    len(item.get("supporting_citation_ids",[]))))
            reg = parsed.get("regulatory_assessment",[])
            for item in reg[:2]:
                print("  reg_item: framework=%r strength=%s cit_count=%d" % (
                    item.get("framework","")[:40], item.get("evidence_strength"),
                    len(item.get("supporting_citation_ids",[]))))
            cc = parsed.get("compliance_checklist",[])
            print("  compliance items with valid citation: %d / %d" % (
                sum(1 for c in cc if c.get("supporting_citation_id")), len(cc)))
